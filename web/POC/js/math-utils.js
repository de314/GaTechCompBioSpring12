/**
 * This is not my code. Pulled from:
 * http://www3.telus.net/thothworks/PolyRootRe.html
 * 
 * Which stated:
 * -------------
 * 		References: The utility posted on this page is a Javascript translation 
 * 		of the FORTRAN routine RPOLY.FOR, which is posted off the NETLIB site as 
 * 		TOMS/493. Although all care has been taken to ensure that the sub-routines 
 * 		were translated accurately, some errors may have crept into the translation. 
 * 		These errors are mine; the original FORTRAN routines have been thoroughly 
 * 		tested and work properly. Please report any errors to the webmaster.
 */

var MAXDEGREE = 100;             // Global variable, degree of largest
									// polynomial accepted by this script.
...
function QuadSD_ak1(NN, u, v, p, q, iPar){
  ...
  // iPar is a dummy variable for passing in the two parameters--a and b--by
	// reference
...
function calcSC_ak1(DBL_EPSILON, N, a, b, iPar, K, u, v, qk){
  ...
  // iPar is a dummy variable for passing in the nine parameters--a1, a3, a7,
	// c, d, e, f, g, and h --by reference

  var sdPar = new Object();    // sdPar is a dummy variable for passing the two
								// parameters--c and d--into QuadSD_ak1 by
								// reference
...
function newest_ak1(tFlag, iPar, a, a1, a3, a7, b, c, d, f, g, h, u, v, K, N, p){
  ...
  // iPar is a dummy variable for passing in the two parameters--uu and vv--by
	// reference
  // iPar.a = uu, iPar.b = vv
...
function Quad_ak1(a, b1, c, iPar){
  ...
  // iPar is a dummy variable for passing in the four parameters--sr, si, lr,
	// and li--by reference
...
function QuadIT_ak1(DBL_EPSILON, N, iPar, uu, vv, qp, NN, sdPar, p, qk, calcPar, K){
  ...
  // sdPar is a dummy variable for passing the two parameters--a and b--in by
	// reference
  // calcPar is a dummy variable for passing the nine parameters--a1, a3, a7,
	// c, d, e, f, g, and h --in by reference

  var qPar = new Object();    // qPar is a dummy variable for passing the four
								// parameters--szr, szi, lzr, and lzi--into
								// Quad_ak1 by reference
...
function RealIT_ak1(DBL_EPSILON, iPar, sdPar, N, p, NN, qp, K, qk){
  ...
  // dumFlag - flag to indicate a pair of zeros near real axis, returned to
	// iFlag
...
function Fxshfr_ak1(DBL_EPSILON, L2, sr, v, K, N, p, NN, qp, u, iPar){
  ...
  var sdPar = new Object();    // sdPar is a dummy variable for passing the two
								// parameters--a and b--into QuadSD_ak1 by
								// reference
  var calcPar = new Object();
  // calcPar is a dummy variable for passing the nine parameters--a1, a3, a7,
	// c, d, e, f, g, and h --into calcSC_ak1 by reference
  ...
  var fflag, i, iFlag = 1, j, spass, stry, tFlag, vpass, vtry;     // Integer
																	// variables
  ...
  for (j = 0; j < L2; j++){
    ...
    // Use sdPar for passing in uu and vv instead of defining a brand-new
	// variable.
	  ...
	  if ((spass) || (vpass)){
		...
		for ( ; ; ) {
		  ...
		  if (iFlag != 0){
		    ...
		    // Use sdPar for passing in s instead of defining a brand-new
			// variable.
		    // sdPar.a = s
...
function rpSolve(dataForm){
 ...
 var dataFormElements = dataForm.elements; // Reference to the form elements
											// array.
 var textareaIndex = 0;  // The form array index for the textarea element
 var textareaString = "";
 var outputString = "";
 var rawArray = new Array(); // Array for holding raw data entry from
								// textarea box
 var rawArrayLength = 0; // The length of the raw array entered from textarea
							// box
 var numCoeff = 0;  // Indicates the number of data entries that have been
					// entered
 var Degree = 0;								// The degree of the polynomial
												// to be solved.
 var k;   // Array index
 ...
 // At this point, textareaString is a big long string containing the entire
	// field entered in the textarea box.
 // Must now clean up the data: remove leading and trailing spaces, identify
	// the inputs, etc.
 ...
 // Check to see that string contains only digits, decimal points, "+" or "-"
	// sign, or white space; if not, no point continuing
 ...
 // if not, then there are not AT LEAST two entries, and the algorithm won't
	// work--no point continuing
 ...
 textareaString = textareaString.replace(/\s*$/, ''); // Remove trailing
														// whitespace
 textareaString = textareaString.replace(/^\s*/, ''); // Remove leading
														// whitespace

 // Divide the string up into its individual entries, which--presumably--are
...
// A maximum of 102 entries may be entered (first entry for the polynomial
// degree, up to 101 for the coefficients).
...
// Now check the individual data entries, confirm they are valid numbers, and
// place the entries in the array
 ...
 if (Degree > MAXDEGREE){
      // alert("This utility accepts polynomials of degree up to " + MAXDEGREE
		// + ". However, a higher number has been input. No further action
		// taken.");
...
if (Degree < 0 ){
 ...
 if (p[0] == 0 ){
      // alert("The leading coefficient of the polynomial was input as 0. No
 ...
 // **********************************************************************************************************************************
 // At this point, Degree should be the polynomial degree and the p array
	// should contain the coefficient values entered
 // **********************************************************************************************************************************
 rpSolve2(Degree, p);
 
 dataForm.erCode.value = Degree;
 
 var r = outputString = outputString + "<br /> &nbsp; &nbsp; The zeros follow:<br /> &nbsp; &nbsp;";

 for (var i = 0; i < Degree; i++) {
   outputString = outputString + r.zeror[i];
   if (r.zeroi[i] != 0) outputString = outputString + " &nbsp; + &nbsp; " + r.zeroi[i] + " i" ;
...
function rpSolve2(Degree, p) {

 var N = Degree;
 var RADFAC = 3.14159265358979323846/180;  // Degrees-to-radians conversion
											// factor = PI/180
 var LB2 = Math.LN2;	   // Dummy variable to avoid re-calculating this value
							// in loop below

 var zeror = new Array(MAXDEGREE);   // Array of real components of roots
 var zeroi = new Array(MAXDEGREE);   // Array of imaginary components of
										// roots
 ...
 var qPar = new Object();    // qPar is a dummy variable for passing the four
								// parameters--sr, si, lr, and li--by reference
 var Fxshfr_Par = new Object();    // Fxshfr_Par is a dummy variable for
									// passing parameters by reference : NZ,
									// lzi, lzr, szi, szr);
 ...
 // To calculate this value, just use existing variables rather than create
	// new ones that will be used only for this code block
 ...
 while (N >= 1){ // Main loop
   ...
   if (((sc <= 1.0) && (moduli_max >= 10)) || ((sc > 1.0) && (Number.MAX_VALUE/sc >= moduli_max))){
	 ...
	 if (factor != 1.0){
	   for (i = 0; i < NN; i++) p[i] *= factor;
		...
		// (Number.MAX_VALUE/sc >= moduli_max)))
   ...
   // Compute the derivative as the initial K polynomial and do 5 steps with no
	// shift
